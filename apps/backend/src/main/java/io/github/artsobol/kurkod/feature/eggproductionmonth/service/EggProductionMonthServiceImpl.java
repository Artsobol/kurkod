...
public class EggProductionMonthServiceImpl implements EggProductionMonthService {
  ...
  public List<EggProductionMonthResponse> getAllByChicken(Long chickenId) {
    return eggProductionMonthRepository.findAllByChicken_IdAndIsActiveTrue(chickenId).stream()
        .map(eggProductionMonthMapper::toResponse)
        .toList();
  }

  @Override
  public List<EggProductionMonthResponse> getAllByChickenAndYear(Long chickenId, int year) {
    return eggProductionMonthRepository
        .findAllByChicken_IdAndYearAndIsActiveTrue(chickenId, year)
        .stream()
        .map(eggProductionMonthMapper::toResponse)
        .toList();
  }
  ...
  public EggProductionMonthResponse create(
  ...
  public EggProductionMonthResponse update(
    ...
    return eggProductionMonthMapper.toResponse(
        eggProductionMonthRepository.save(eggProductionMonth));
  }
  ...
  protected EggProductionMonth findByIdMonthYear(Long chickenId, int month, int year) {
    return eggProductionMonthRepository
        .findByChicken_IdAndMonthAndYearAndIsActiveTrue(chickenId, month, year)
        .orElseThrow(
            () -> new NotFoundException("egg.production.not.found", chickenId, month, year));
  }
  ...
  protected boolean existsByIdMonthYear(Long chickenId, int month, int year) {
    return eggProductionMonthRepository.existsByChicken_IdAndMonthAndYearAndIsActiveTrue(
        chickenId, month, year);
  }
}
