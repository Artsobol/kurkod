...
public class EmploymentContractServiceImpl implements EmploymentContractService {
  ...
  protected EmploymentContract getContractByWorkerId(Long workerId) {
    return employmentContractRepository
        .findEmploymentContractByWorkerIdAndIsActiveTrue(workerId)
        .orElseThrow(
            () -> new NotFoundException("employment.contract.not.found.by.worker", workerId));
  }

  protected Staff getStaffByStaffId(Long staffId) {
    return staffRepository
        .findStaffByIdAndIsActiveTrue(staffId)
