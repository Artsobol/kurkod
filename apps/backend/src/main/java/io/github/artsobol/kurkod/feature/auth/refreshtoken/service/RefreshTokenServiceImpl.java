...
public class RefreshTokenServiceImpl implements RefreshTokenService {
  ...
  @Override
  @Transactional
  public RefreshTokenRotationResponse rotate(RotateRefreshTokenRequest request) {
    ...
    CreateRefreshTokenRequest refreshTokenRequest =
  ...
  public void revoke(String rawRefreshToken) {
    ...
    refreshTokenRepository
        .findByTokenHash(hash)
        .ifPresent(
            token -> revokeSession(token.getUser().getId(), token.getSessionId(), Instant.now()));
  }
  ...
  private void ensureHasSessions(Long userId, long sessionsCount) {
    if (sessionsCount >= properties.maxSessions()) {
      log.info(
          "User: {} has too many active sessions={}, revoking oldest one", userId, sessionsCount);
      RefreshToken token =
          refreshTokenRepository.findOldestActiveSessions(userId, PageRequest.of(0, 1)).getFirst();
  ...
  private void revokeSession(Long userId, UUID sessionId, Instant now) {
    List<RefreshToken> activeTokens =
        refreshTokenRepository.findActiveByUserIdAndSessionId(userId, sessionId);
