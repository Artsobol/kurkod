...
public class CageServiceImpl implements CageService {
  ...
  @Transactional
  @PreAuthorize("hasAnyAuthority('DIRECTOR', 'SUPER_ADMIN')")
  public CageResponse update(
      Long rowId, Integer cageNumber, CageUpdateRequest cageUpdateRequest, Long version) {
  ...
  protected Cage findCageByRowIdAndCageNumber(Long rowId, Integer cageNumber) {
    return cageRepository
        .findByRow_IdAndCageNumberAndIsActiveTrue(rowId, cageNumber)
        .orElseThrow(() -> new NotFoundException("cage.not.found.by.keys", rowId, cageNumber));
  }
